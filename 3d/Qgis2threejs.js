...
Q3D.Scene.prototype.adjustZShift = function () {
  ...
  var zmin =
    box.min.z === Infinity
      ? 0
      : box.min.z / this.userData.zScale - this.userData.zShift;
...
(function () {
  ...
  app.init = function (container) {
    ...
    if (params.anisotropy)
      Q3D.Config.texture.anisotropy =
        parseFloat(params.anisotropy) || Q3D.Config.texture.anisotropy;

    if (params.cx !== undefined)
      Q3D.Config.viewpoint.pos.set(
        parseFloat(params.cx),
        parseFloat(params.cy),
        parseFloat(params.cz)
      );
    if (params.tx !== undefined)
      Q3D.Config.viewpoint.lookAt.set(
        parseFloat(params.tx),
        parseFloat(params.ty),
        parseFloat(params.tz)
      );
    ...
    if (Q3D.Config.texture.anisotropy === null)
      Q3D.Config.texture.anisotropy =
        app.renderer.capabilities.getMaxAnisotropy() || 1;
    ...
    app.scene.labelRootElement.style.display = app.labelVisible
      ? 'block'
      : 'none';
    ...
    app.queryMarker = new THREE.Mesh(
      new THREE.SphereBufferGeometry(opt.r, 32, 32),
    ...
    app.renderer.domElement.addEventListener(
      'mousedown',
      app.eventListener.mousedown
    );
    app.renderer.domElement.addEventListener(
      'mouseup',
      app.eventListener.mouseup
    );
  ...
  app.parseUrlParameters = function () {
    ...
    var params = window.location.search
      .substring(1)
      .split('&')
      .concat(window.location.hash.substring(1).split('&'));
  ...
  app.initOrbitControls = function (camera, domElement) {
    ...
    controls.keyRotateAngle =
      (Q3D.Config.controls.keyRotateSpeed * Math.PI) / 180;
    ...
    controls.cameraRotate = function (thetaDelta, phiDelta) {
      ...
      spherical.theta = Math.max(
        ...
        Math.min(controls.maxAzimuthAngle, spherical.theta)
      );
  ...
  app.initLoadingManager = function () {
    if (app.loadingManager) {
      app.loadingManager.onLoad =
        app.loadingManager.onProgress =
        app.loadingManager.onError =
          undefined;
    }
  ...
  app.loadFile = function (url, type, callback) {
    ...
    var onError = function (e) {
      if (location.protocol == 'file:') {
        app.popup.show(
          "This browser doesn't allow loading local files via Ajax. See <a href='https://github.com/minorua/Qgis2threejs/wiki/Browser-Support'>plugin wiki page</a> for details.",
          'Error',
          true
        );
      }
  ...
  app.loadSceneFile = function (
    url,
    sceneFileLoadedCallback,
    sceneLoadedCallback
  ) {
    var onload = function () {
      ...
      if (Q3D.Config.northArrow.visible)
        app.buildNorthArrow(
          ...
          app.scene.userData.baseExtent.rotation
        );
  ...
  app.loadModelData = function (data, ext, resourcePath, callback) {
    if (ext == 'dae') {
      var model = new THREE.ColladaLoader(app.loadingManager).parse(
        data,
        resourcePath
      );
      if (callback) callback(model);
    } else if (ext == 'gltf' || ext == 'glb') {
      new THREE.GLTFLoader(app.loadingManager).parse(
        data,
        resourcePath,
        function (model) {
          if (callback) callback(model);
  ...
  app.buildNorthArrow = function (container, rotation) {
    ...
    app.camera2 = new THREE.PerspectiveCamera(
      45,
    ...
    var material = new THREE.MeshLambertMaterial({
      color: Q3D.Config.northArrow.color,
  ...
  app.render = function (updateControls) {
    ...
    if (app.renderer2) {
      ...
      app.camera2.position.copy(
        vec3.negate().setLength(Q3D.Config.northArrow.cameraDistance)
      );
    ...
    app.updateLabelPosition = function () {
      ...
      for (i = 0, l = rootGroup.children.length; i < l; i++) {
        ...
        for (k = 0, m = connGroup.children.length; k < m; k++) {
          ...
          if (c2l.subVectors(vec3, camera.position).dot(c2t) > 0)
            // label is in front
            obj_dist.push([conn, pt0, camera.position.distanceTo(vec3)]);
          // label is in back
          else conn.userData.elem.style.display = 'none';
        }
      }
  ...
  app.showQueryResult = function (point, obj, show_coords) {
    ...
    if (e) {
      if (show_coords) {
        ...
        if (Q3D.Config.coord.latlon) {
          ...
          e.innerHTML =
            Q3D.Utils.convertToDMS(lonLat[1], lonLat[0]) +
            ', Elev. ' +
            pt.z.toFixed(2);
        } else {
          e.innerHTML = [
    ...
    if (e) {
      for (var i = e.children.length - 1; i >= 0; i--) {
        if (e.children[i].tagName.toUpperCase() == 'TR')
  ...
  app.highlightFeature = function (object) {
    ...
    if (['Icon', 'JSON model', 'COLLADA model'].indexOf(layer.objType) != -1)
      return;
  ...
  app.saveCanvasImage = function (
    ...
    var saveBlob = function (blob) {
      ...
      if (window.navigator.msSaveBlob !== undefined) {
        ...
        e.innerHTML = 'Save';
        app.popup.show(
          'Click to save the image to a file.' + e.outerHTML,
          'Image is ready'
        );
      }
    };
...
Q3D.Material.prototype = {
  ...
  loadJSONObject: function (jsonObject, callback) {
    ...
    if (m.image !== undefined) {
      ...
      if (m.image.url !== undefined) {
        ...
        defer = true;
      } else if (m.image.object !== undefined) {
        // WebKit Bridge
  ...
  type: function () {
    if (this.mtl instanceof THREE.MeshLambertMaterial)
      return Q3D.MaterialType.MeshLambert;
    if (this.mtl instanceof THREE.MeshPhongMaterial)
      return Q3D.MaterialType.MeshPhong;
    if (this.mtl instanceof THREE.LineBasicMaterial)
      return Q3D.MaterialType.LineBasic;
    if (this.mtl instanceof THREE.LineDashedMaterial)
      return Q3D.MaterialType.LineDashed;
    if (this.mtl instanceof THREE.SpriteMaterial)
      return Q3D.MaterialType.Sprite;
    if (this.mtl instanceof THREE.MeshStandardMaterial)
      return Q3D.MaterialType.MeshStandard;
...
Q3D.Materials.prototype.addFromObject3D = function (object) {
  ...
  object.traverse(function (obj) {
    ...
    (obj.material instanceof Array ? obj.material : [obj.material]).forEach(
      function (mtl) {
...
Q3D.Materials.prototype.setWireframeMode = function (wireframe) {
  ...
  for (var i = 0, l = this.materials.length; i < l; i++) {
    material = this.materials[i];
    if (material.origProp.w || material.mtl instanceof THREE.LineBasicMaterial)
      continue;
...
Q3D.DEMBlock.prototype = {
  ...
  loadJSONObject: function (obj, layer, callback) {
    ...
    if (layer.geometryCache) {
      ...
      if (
        ...
        params.widthSegments === grid.width - 1 &&
        params.heightSegments === grid.height - 1
      ) {
  ...
  buildSides: function (layer, parent, material, z0) {
    ...
    var e0 =
        z0 / this.data.zScale - this.data.zShift - (this.data.zShiftA || 0),
      band_width = -2 * e0;

    // front and back
    var geom_fr = new THREE.PlaneBufferGeometry(
        planeWidth,
        band_width,
        w - 1,
        1
    ...
    // left and right
    var geom_le = new THREE.PlaneBufferGeometry(
        band_width,
        planeHeight,
        1,
        h - 1
  ...
  getValue: function (x, y) {
    ...
    if (0 <= x && x < grid.width && 0 <= y && y < grid.height)
      return grid.array[x + grid.width * y];
    return null;
  },
  ...
  loadJSONObject: function (obj, layer, callback) {
    ...
    var buildGeometry = function (obj) {
      ...
      for (var i = 0, l = vertices.length; i < l; i += 3) {
        normals.push(0, 0, 1);
        uvs.push(
          vertices[i] / base_width + 0.5,
          vertices[i + 1] / base_height + 0.5
        );
      }
...
Q3D.MapLayer = function () {
  ...
  this.materials.addEventListener(
    'renderRequest',
...
Q3D.MapLayer.prototype.loadJSONObject = function (jsonObject, scene) {
  if (jsonObject.type == 'layer') {
    ...
    if (jsonObject.properties !== undefined) {
      ...
      this.visible =
        jsonObject.properties.visible || Q3D.Config.allVisible ? true : false;
    }
...
Q3D.DEMLayer.prototype.buildBlock = function (jsonObject, scene) {
  var _this = this,
    block =
      jsonObject.grid !== undefined
        ? new Q3D.DEMBlock()
        : new Q3D.ClippedDEMBlock();
  ...
  block.loadJSONObject(jsonObject, this, function (mesh) {
    var addEdges = function () {
      ...
      block.addEdges(
        ...
        mesh,
        material.mtl,
        jsonObject.sides ? Q3D.Config.dem.side.bottomZ : undefined
      );
    };
...
Q3D.DEMLayer.prototype.segmentizeLineString = function (lineString, zFunc) {
  ...
  pts.push(
    new THREE.Vector3(
      pt[0],
      pt[1],
      pt[2] === undefined ? zFunc(pt[0], pt[1]) : pt[2]
    )
  );
...
Q3D.VectorLayer.prototype.buildLabels = function (features, getPointsFunc) {
  if (this.properties.label === undefined || getPointsFunc === undefined)
    return;
  ...
  var line_mat = new THREE.LineBasicMaterial({
    color: Q3D.Config.label.connectorColor,
  });
...
Q3D.VectorLayer.prototype.loadJSONObject = function (jsonObject, scene) {
  ...
  if (jsonObject.type == 'layer') {
    if (jsonObject.data !== undefined) {
      ...
      if (this.properties.label !== undefined) {
        ...
        if (this.labelParentElement === undefined) {
          ...
          this.labelParentElement.style.display = this.visible
            ? 'block'
            : 'none';
      ...
      (jsonObject.data.blocks || []).forEach(function (block) {
        if (block.url !== undefined) Q3D.application.loadJSONFile(block.url);
          ...
          if (this.properties.label !== undefined)
            this.buildLabels(block.features);
        }
      }, this);
    }
  } else if (jsonObject.type == 'block') {
    ...
    if (this.properties.label !== undefined)
      this.buildLabels(jsonObject.features);
  }
};
...
Object.defineProperty(Q3D.VectorLayer.prototype, 'visible', {
  ...
  set: function (value) {
    if (this.labelParentElement)
      this.labelParentElement.style.display = value ? 'block' : 'none';
    if (this.labelConnectorGroup) this.labelConnectorGroup.visible = value;
    Object.getOwnPropertyDescriptor(Q3D.MapLayer.prototype, 'visible').set.call(
      this,
      value
    );
  },
...
Q3D.PointLayer.prototype.loadJSONObject = function (jsonObject, scene) {
  if (
    jsonObject.type == 'layer' &&
    jsonObject.properties.objType == 'Model File' &&
...
Q3D.PointLayer.prototype.build = function (features) {
  ...
  if (objType == 'Point') {
    this.buildPoints(features);
    return;
  }
  if (objType == 'Icon') {
    this.buildIcons(features);
    return;
  }
  if (objType == 'Model File') {
    this.buildModels(features);
    return;
  }
  ...
  if (objType == 'Sphere') {
    ...
    unitGeom =
      ...
      (objType == 'Cylinder'
        ...
        : new THREE.CylinderBufferGeometry(0, 1, 1, 32));
  }
...
Q3D.PointLayer.prototype.buildLabels = function (features) {
  Q3D.VectorLayer.prototype.buildLabels.call(this, features, function (f) {
    return f.geom.pts;
  });
};
...
Q3D.LineLayer.prototype.build = function (features) {
  ...
  if (objType == this._lastObjType && this._createObject !== undefined) {
    ...
    createObject = function (f, line) {
      ...
      if (obj.material instanceof THREE.LineDashedMaterial)
    ...
    createObject = function (f, line) {
      ...
      for (var i = 1, l = line.length; i < l; i++) {
        ...
        mesh.quaternion.setFromUnitVectors(
          axis,
          sub.subVectors(pt1, pt0).normalize()
        );
        group.add(mesh);
    ...
    createObject = function (f, line) {
      ...
      var bzFunc = function (x, y) {
        return z0 + f.geom.bh;
      };
      return new THREE.Mesh(
        Q3D.Utils.createWallGeometry(vertices, bzFunc),
        materials.mtl(f.mtl)
      );
    };
  }
...
Q3D.PolygonLayer.prototype.build = function (features) {
  ...
  if (
    this.properties.objType == this._lastObjType &&
    this._createObject !== undefined
  ) {
    createObject = this._createObject;
    ...
    createObject = function (f) {
      ...
      geom.setAttribute(
        'position',
        new THREE.Float32BufferAttribute(f.geom.triangles.v, 3)
      );
    ...
    var createSubObject = function (f, polygon, z) {
      ...
      for (i = 1, l = polygon.length; i < l; i++) {
        shape.holes.push(
          new THREE.Path(Q3D.Utils.arrayToVec2Array(polygon[i]))
        );
      }

      // extruded geometry
      var geom = new THREE.ExtrudeBufferGeometry(shape, {
        bevelEnabled: false,
        depth: f.geom.h,
      });
      ...
      if (f.mtl.edge !== undefined) {
        ...
        for (i = 0, l = polygon.length; i < l; i++) {
          ...
          for (j = 0, m = bnd.length - 1; j < m; j++) {
            v = [bnd[j][0], bnd[j][1], 0, bnd[j][0], bnd[j][1], h];
    ...
    createObject = function (f) {
      if (f.geom.polygons.length == 1)
        return createSubObject(f, f.geom.polygons[0], f.geom.centroids[0][2]);

      var group = new THREE.Group();
      for (var i = 0, l = f.geom.polygons.length; i < l; i++) {
        group.add(
          createSubObject(f, f.geom.polygons[i], f.geom.centroids[i][2])
        );
      }
      return group;
    };
      ...
      if (f.geom.brdr !== undefined) {
        ...
        for (i = 0, l = f.geom.brdr.length; i < l; i++) {
          ...
          for (j = 0, m = bnds.length; j < m; j++) {
            ...
            geom.setAttribute(
              'position',
              new THREE.Float32BufferAttribute(bnds[j], 3)
            );
...
Q3D.Model.prototype = {
  ...
  loadJSONObject: function (jsonObject, callback) {
    if (jsonObject.url !== undefined) {
      ...
      this.loadData(
        atob(jsonObject.base64),
        jsonObject.ext,
        jsonObject.resourcePath,
        callback
      );
    }
  },
...
Q3D.Utils.putStick = function (x, y, zFunc, h) {
  ...
  if (zFunc === undefined) {
    zFunc = function (x, y) {
      return Q3D.application.scene.mapLayers[0].getZ(x, y);
    };
  }
  var z = zFunc(x, y);
...
Q3D.Utils.setGeometryUVs = function (geom, base_width, base_height) {
  ...
  for (var i = 0, l = geom.vertices.length; i < l; i++) {
    v = geom.vertices[i];
    uvs.push(
      new THREE.Vector2(v.x / base_width + 0.5, v.y / base_height + 0.5)
    );
  }
