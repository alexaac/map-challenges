...
function ViewHelper(editorCamera, container) {
  ...
  var geometry = new THREE.BoxBufferGeometry(0.8, 0.05, 0.05).translate(
    0.4,
    0,
    0
  );
  ...
  this.update = function (delta) {
    ...
    if (
      dummy.quaternion.angleTo(targetQuaternion) < 0.01 &&
      editorCamera.quaternion.angleTo(targetQuaternion) < 0.01
    ) {
  ...
  function prepareAnimationData(object, focusPoint) {
    ...
    if (point.angleTo(ez) < 0.01) {
      // +z
      dummy.rotateOnAxis(enz, Math.PI * 0.5);
    } else if (point.angleTo(enz) < 0.01) {
      // -z
